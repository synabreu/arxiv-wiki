...
arXiv의 신규 AI 논문을 매일 수집하고 Top 10을 선별한 뒤, 일반 사용자와 개발자가 읽을 수 있는 한국어 Markdown 문서로 생성하는 프로젝트다.

## 제공 기능

- `cs.AI`, `cs.CL`, `cs.LG`, `cs.CV`, `cs.SE`, `cs.DC` 신규 논문 수집
- 최근성, 분야, LLM·에이전트·추론 등 핵심어 기반 Top 10 선별
...
현재 Top 10은 다음 신호의 휴리스틱 점수로 결정한다.

- 등록 시점의 최근성
- AI·ML·NLP 카테고리 가중치
- LLM, agent, reasoning, inference, multimodal, MoE, RAG 등 핵심어
- 구체적인 초록과 오픈소스·성능 비교 신호

이 점수는 논문 품질이나 인용 영향력을 확정하는 지표가 아니다. 신규 논문은 인용 수와 커뮤니티 반응이 아직 없기 때문에 일일 큐레이션 목적의 관심도 점수로 사용한다.
